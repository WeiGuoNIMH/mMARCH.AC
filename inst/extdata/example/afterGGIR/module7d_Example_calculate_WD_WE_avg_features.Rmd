...
MeanWithMask<-function(Fmer5.N.all,Fmer5,QCnights.allday,QCnights=0){ 
...
```{r c3AvgWithMask,warning=FALSE,error=FALSE,echo=FALSE} 
# "filename"  "Date" "newID" "cleaningcode" 
#  [5] "sleeponset" "wakeup"   .... "IV_intradailyvariability"   "IV"   ("IS" ..... "PC10") 
#  [93]"weekday" "zeroSleep"  "Nwd"  "Nwe" Nwd_0s"    "Nwe_0s"
...
DayFeature<-read.csv(inFN,head=1,stringsAsFactors=F)
wdwefea<-read.csv(paste("module7d_",studyname,"_new_features_page1.csv",sep=""),head=1,stringsAsFactors=F)   
pcFN<-paste("module7d_",ftitle,"_",studyname,"_new_features_page5_denseFLMM_subject_PCs.csv",sep="")


C1<-which(colnames(DayFeature)=="IS") 
C2<-which(colnames(DayFeature)=="PC10") 
C3<-which(colnames(DayFeature)=="sleeponset") 
C4<-which(colnames(DayFeature)=="Nblocks_spt_sleep") 
C5<-which(colnames(DayFeature)=="weekday") 
S4<-setdiff(1:ncol(DayFeature),C1:C2) #rm subject level PA/CR features 
S4a<-c(1,C3:C4,C5:ncol(DayFeature))   #sleep features
S4b<-c(1,(C4+1):(C1-1),C5:ncol(DayFeature)) # nonsleep daylevel features
...
Fmer5.N.all<-aggregate(.~filename,data=Fmer5[, c(1,B:(Ncol8-1))], NcountSum,na.action=na.pass) 
tailname<-colnames(Fmer5)[Ncol8:ncol(Fmer5)] 
 
H<-holidayplusweekends(DayFeature,holidayFN=holidayFN)      
#f=1/2 for WD/WE................................................
for (f in 1:2){
...
sleepWD<-DayFeature[sleepSet,S4a]   
paWD<-DayFeature[paSet,S4b]   
SleepData<-MeanWithMask(Fmer5.N.all,Fmer5=sleepWD,QCnights.allday=QCnights.feature.alpha[1],QCnights=QCnights)   
PAData<-MeanWithMask(Fmer5.N.all,Fmer5=paWD,QCnights.allday=QCnights.feature.alpha[1],QCnights=QCnights)  
...
#x4) merage new daylevel feature + wdwe subject leve + wdwePCs
...
R -e "rmarkdown::render('module7d_Example_calculate_WD_WE_avg_features.Rmd'   )" 
