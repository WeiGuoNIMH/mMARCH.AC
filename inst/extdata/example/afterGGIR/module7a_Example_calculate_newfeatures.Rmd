...
```{r chunk21} 
tac24 = Tvol2(count.data = count, weartime = wear,logtransform = FALSE)  
...
ra_all = RA_long2(count.data = count, window=(ncol(count)-2)/1440, method = "average") # one minute window  
...
end3<- as.integer((ra_all[i,"M10TIME"]+10-1/60)*60)  #end position for 1440 data  
end3<-end3*60/epochOut 
start3<-end3-n599   
  
ra_all[i,"start"]<-start3
ra_all[i,"end"]<-end3
...
Cos = ActCosinor_long2( count.data = count, window=minuteW)  
Cos.log = ActCosinor_long2( count.data = log.count, window=minuteW)   
extcos = ActExtendCosinor_long2(count.data = count, window=minuteW)  
extcos.log = ActExtendCosinor_long2(count.data = log.count, window=minuteW)   
...
ra_all = RA_long2(count.data = count, window=(ncol(count)-2)/1440, method = "average") # one minute window for RA,L5,M10,L5time, M10time 
```
...
R -e "rmarkdown::render('module7a_Example_calculate_newfeatures.Rmd'   )" 
