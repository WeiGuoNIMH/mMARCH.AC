...
gpart24.sel<-gpart4[,S1] 
gpart24.sel[,"calendardate"]<- format(as.Date(gpart24.sel[,"calendardate"],"%d/%m/%Y"), "%Y-%m-%d")
...
quantile(as.numeric(part5data.clean[,"L10TIME_num"]),na.rm=T)
quantile(as.numeric(part5data.clean[,"M10TIME_num"]),na.rm=T)
quantile(as.numeric(part5data.clean[,"dur_day_total_IN_min"]),na.rm=T)
colnames(part5data.clean)[colnames(part5data.clean)=="calendar_date"]<- "calendardate"  #6/5/2020 
part5data.clean[,"RA_ggir"]<-(part5data.clean[,"M10VALUE"]- part5data.clean[,"L5VALUE"])/(part5data.clean[,"M10VALUE"]+ part5data.clean[,"L5VALUE"])  
...
S3<-c("filename", intersect(part2variable ,colnames(gpart2))   ) 
gpart2.clean<-gpart2[,S3] 
...
ggirmer0<-merge(gpart24.sel,part5data.clean.sel,by=c("filename","calendardate"),all=TRUE)
ggirmer0[,"filename"]<-gsub(".RData","",ggirmer0[,"filename"])  
ggirmer<-merge(ggirmer0,gpart2.clean,by=c("filename" ),all.x=TRUE,all.y=FALSE)  
dim(gpart24.sel)
...
Fmer81<-aggregate(.~filename,data=Fmer6[, c(1,B:(Ncol8-1))], mean,na.rm=T,na.action=na.pass) #head
Fmer82<-aggregate(.~filename,data=Fmer6[,c(1,Ncol8:ncol(Fmer6))], sum,na.rm=T,na.action=na.pass) #tail
timingC<-grep("cmean",colnames(Fmer6)) 
Fmer83<-aggregate(.~filename,data=Fmer6[, c(1,timingC)], get_mean_sd_hour,unit2minute=60,out="mean",na.action=na.pass)  #L5TIME
